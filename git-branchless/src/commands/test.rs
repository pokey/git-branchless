...
use lib::core::effects::{Effects, OperationIcon, OperationType};
use lib::core::eventlog::{EventLogDb, EventReplayer, EventTransactionId};
use lib::core::formatting::{printable_styled_string, Pluralize, StyledStringBuilder};
use lib::core::repo_ext::RepoExt;
use lib::core::rewrite::{
    execute_rebase_plan, ExecuteRebasePlanOptions, ExecuteRebasePlanResult, RebaseCommand,
    RebasePlan,
};
use lib::git::{Commit, GitRunInfo, Repo};
    ...
    fn describe(&self, effects: &Effects, commit: &Commit) -> eyre::Result<StyledString> {
        ...
        let description = match self {
            TestOutput::CheckoutFailed => StyledStringBuilder::new()
                .append_styled("⚠️ Failed to check out: ", *STYLE_SKIPPED)
                .append(commit.friendly_describe(glyphs)?)
                .build(),
            TestOutput::SpawnTestFailed(err) => StyledStringBuilder::new()
                .append_styled(format!("⚠️ Failed to spawn test: {err}: "), *STYLE_SKIPPED)
                .append(commit.friendly_describe(glyphs)?)
                .build(),
            ...
            TestOutput::Failed(exit_code) => StyledStringBuilder::new()
                .append_styled(
                    format!("✖️ Failed with exit code {exit_code}: "),
                    *STYLE_FAILURE,
                )
                .append(commit.friendly_describe(glyphs)?)
                .build(),
            TestOutput::Passed => StyledStringBuilder::new()
                .append_styled(format!("✔️ Passed: "), *STYLE_SUCCESS)
                .append(commit.friendly_describe(glyphs)?)
                .build(),
        };
        Ok(description)
...
fn run_tests(
    ...
    let results = {
        ...
        for commit in commits {
            {
                let (effects, progress) = effects.start_operation(OperationType::RunTestOnCommit(
                    Arc::new(printable_styled_string(
                        effects.get_glyphs(),
                        commit.friendly_describe(effects.get_glyphs())?,
                    )?),
                ));
                let output = match prepare_working_directory(
                    &effects,
                    git_run_info,
                    repo,
                    event_tx_id,
                ...
                progress.notify_status(
                    ...
                    printable_styled_string(effects.get_glyphs(), text)?,
                );
    ...
    for (commit, output) in results {
        writeln!(
            effects.get_output_stream(),
            "{}",
            printable_styled_string(effects.get_glyphs(), output.describe(effects, commit)?)?
        )?;
        match output {
...
fn prepare_working_directory(
    ...
    let checkout_exit_code = git_run_info.run(
        &effects,
        Some(event_tx_id),
        &["checkout", &commit.get_oid().to_string()],
    )?;
    if checkout_exit_code.is_success() {
